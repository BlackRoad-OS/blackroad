...
1. Wake Up: Load oracle_state.json from disk.
2. Scavenge: Ping the model_registry.yaml to see which APIs are valid today.
3. Execute: Perform task using the cheapest/smartest available model.
4. Persist: Write result to oracle_state.json.
5. Sleep: Terminate process. (The agent is "dead" until next run; no lingering cloud state).
