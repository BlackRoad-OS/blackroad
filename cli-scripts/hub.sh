...
_draw_hub() {
    ...
    echo -e "${B}║${R}  ${W}[D]${R}ocker ps  ${W}[C]${R}ompose↑  ${W}[X]${R}ompose↓  ${W}[I]${R}mages  ${W}[S]${R}tats  ${W}[e]${R}cece  ${W}[v]${R}ault  ${W}[A]${R}lert  ${W}[G]${R}it  ${W}[W]${R}s  ${W}[N]${R}snip  ${B}║${R}"
    echo -e "${B}║${R}                                                                            ${B}║${R}"
    echo -e "${B}╚══════════════════════════════════════════════════════════════════════════════╝${R}"
}
...
while true; do
    ...
    if read -t 1.5 -n 1 key 2>/dev/null; then
        case "$key" in
            ...
            N) tput cnorm 2>/dev/null
               echo -e "\n  ${AMBER}◆${NC} Quick-save snippet"
               printf "  name: "; read -r _sname
               printf "  code (single line): "; read -r _scode
               echo "$_scode" | zsh "${SCRIPT_DIR}/../tools/snippet-manager/br-snippet.sh" save "${_sname:-snippet-$(date +%s)}" 2>/dev/null
