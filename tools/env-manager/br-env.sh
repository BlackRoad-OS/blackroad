...
ENVTEMPLATE
    ...
    echo -e "${CYAN}💡 Edit with: br env edit $file${NC}"
}
...
cmd_list() {
    ...
    echo -e "${CYAN}📋 Environment Variables in $file:${NC}\n"
    ...
    while IFS='=' read -r key value; do
        ...
        if [[ "$key" =~ (SECRET|PASSWORD|TOKEN|KEY|PRIVATE) ]]; then
            echo -e "${YELLOW}🔒${NC} $key=${BLUE}****${NC}"
        else
            echo -e "${GREEN}●${NC} $key=${BLUE}$value${NC}"
        fi
    done < "$file"
}
...
cmd_get() {
    ...
    if [[ -z "$key" ]]; then
        echo -e "${RED}❌ Specify variable name${NC}"
        exit 1
    fi
...
cmd_set() {
    ...
    if [[ -z "$key" || -z "$value" ]]; then
        echo -e "${RED}❌ Usage: br env set <file> <key> <value>${NC}"
        exit 1
    fi
    
    if [[ ! -f "$file" ]]; then
        echo -e "${YELLOW}⚠️  File $file not found, creating...${NC}"
        touch "$file"
    fi
...
cmd_copy() {
    ...
    if [[ -z "$dest" ]]; then
        echo -e "${RED}❌ Usage: br env copy <source> <dest>${NC}"
        exit 1
    fi
    ...
    if [[ "$clear" =~ ^[Yy]$ ]]; then
        ...
        echo -e "${GREEN}✓ Cleared secrets in $dest${NC}"
    fi
}
...
cmd_diff() {
    ...
    echo -e "${CYAN}🔍 Differences between $file1 and $file2:${NC}\n"
...
cmd_validate() {
    ...
    while IFS='=' read -r key value; do
        [[ "$key" =~ ^#.*$ || -z "$key" ]] && continue
        if [[ -z "$value" ]]; then
            echo -e "${YELLOW}⚠️  Empty value: $key${NC}"
            errors=$((errors + 1))
        fi
    done < "$file"
    ...
    if [[ -f "$template" ]]; then
        while IFS='=' read -r key _; do
            [[ "$key" =~ ^#.*$ || -z "$key" ]] && continue
            if ! grep -q "^${key}=" "$file"; then
                echo -e "${RED}❌ Missing required: $key${NC}"
                errors=$((errors + 1))
            fi
        done < "$template"
    fi
...
cmd_export() {
    ...
    echo -e "${CYAN}📤 Exporting variables from $file...${NC}\n"
    
    while IFS='=' read -r key value; do
        [[ "$key" =~ ^#.*$ || -z "$key" ]] && continue
        export "$key"="$value"
        echo -e "${GREEN}✓${NC} Exported $key"
    done < "$file"
}
